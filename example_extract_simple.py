...
import sys
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s [%(levelname)s] [%(filename)s:%(lineno)d] %(message)s', stream=sys.stdout)
...
entities = extract_and_link_entities(
    text,
    {
        ...
        # === OUTPUT SETTINGS ===
        "OUTPUT_DIR": os.path.join(os.path.dirname(__file__), "output"),  # Ausgabeverzeichnis
